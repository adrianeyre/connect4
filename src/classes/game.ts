...
export default class Game implements IGame {
  ...
  constructor(_config: IConnect4Props) {
  ...
  public handleInput = (x: number): void => {
    const counters = this.sprites.filter(
      (sprite: ISprite) => sprite.x === x && sprite.type !== SpriteTypeEnum.Empty,
    );
  ...
  private newSprite = (key: string, x: number, y: number) =>
    new Sprite({
      key,
      visable: true,
      x,
  ...
  private currentPlayer = (): SpriteTypeEnum =>
    this.playerOn ? SpriteTypeEnum.Player1 : SpriteTypeEnum.Player2;
  ...
  private findSprite = (x: number, y: number, player: SpriteTypeEnum): ISprite | undefined =>
    this.sprites.find(
      (sprite: ISprite) => sprite.x === x && sprite.y === y && sprite.type === player,
    );
}
